...
CUSTOMER_PARQUET_PATH = Path("C:\\Users\\Divini\\Documents\\projetos\\projeto etl1\\output data\\customer_clean.parquet")
ITEM_PARQUET_PATH = Path("C:\\Users\\Divini\\Documents\\projetos\\projeto etl1\\output data\\item_clean.parquet")
...
def load_data() -> pd.DataFrame:
    # Lê os dois arquivos Parquet
    df_customer = pd.read_parquet("../output data/customer_clean.parquet")
    df_item = pd.read_parquet("../output data/item_clean.parquet")
    
    # Exibe no terminal as colunas encontradas para você conferir
    print("Colunas em customer:", df_customer.columns.tolist())
    print("Colunas em item:", df_item.columns.tolist())
    
    # Garante conversão da chave de junção
...
def filter_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filtro de Faixa Etária (Multiselect no lugar do Slider de Idade)
    ...
    # Filtro de Estação do Ano (Season)
    ...
    # Aplicação dos filtros usando máscaras booleanas corretas para as colunas existentes
    filtered_df = df[
...
def main() -> None:
    ...
    try:
        ...
        st.error("Arquivos Parquet não encontrados. Verifique os caminhos no código.")
        st.stop()
    ...
    with tab_categoria:
        cat_df = (
            filtered_df.groupby("Category", as_index=True)
            .agg(
    ...
    with tab_demografico:
        ...
        gender_df = filtered_df.groupby("Gender")["Purchase Amount (BRL)"].sum()
